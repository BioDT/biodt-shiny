...
generate_chart <- function(
  ...
  chart$x$opts <-
    list(
      title = list(text = "Grassland & Weather simulation"),
      tooltip = list(
        trigger = "axis",
        formatter = formatter,
        axisPointer = list(
...
# Run -----
generate_chart(
  filepaths_grass = filepaths_results,
  filepath_weather = weather_filepath,
  grass_end_date = grass_end_date,
  return_series = FALSE
)
