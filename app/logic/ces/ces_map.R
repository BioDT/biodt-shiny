...
ces_leaflet_map <- function(
  ...
  leaflet_map <- leaflet(
    ...
    addLegend(
      pal = biodiversity_palette(), values = c(0, 1), title = "Biodiversity", position = "bottomleft", # TODO add i18n$translate
      labFormat = labelFormat(prefix = "", suffix = "", between = " - ")
    ) |>
    addLegend(pal = recre_palette(), values = c(0,1), title = "Recreation", position = "bottomleft") |> # TODO add i18n$translate
    # addTiles(
