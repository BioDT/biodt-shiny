...
ias_app_server <- function(id, tab_selected) {
  ...
  filtered_summary <- reactive({
    ...
    req(input$timeFramePicker, input$habNamePicker, input$dataTypePicker)
  ...
  output$selectedOptions <- renderUI({
    ...
    # Habitat
    boxes[[length(boxes)+1]] <- column(width = 4,
                                       div(class="summary-box habitat-box",
                                           icon("leaf"), span("Habitat: ", strong(input$habNamePicker))
                                       )
    )
    # Data Type
    boxes[[length(boxes)+1]] <- column(width = 4,
                                       div(class="summary-box data-type-box",
                                           icon("chart-bar"),
                                           span("Data Type: ", strong(switch(input$dataTypePicker,
                                                                             "mean" = "Mean", "sd" = "Standard Deviation", "cov" = "Uncertainty")))
                                       )
    )
    # Time Frame
    boxes[[length(boxes)+1]] <- column(width = 4,
                                       div(class="summary-box time-frame-box",
    ...
    boxes[[length(boxes)+1]] <- column(width = 4,
                                       div(class="summary-box time-period-box",
    ...
    boxes[[length(boxes)+1]] <- column(width = 4,
                                       div(class="summary-box climate-model-box",
    ...
    boxes[[length(boxes)+1]] <- column(width = 4,
                                       div(class="summary-box climate-scenario-box",
    ...
    if (input$showSpecies && !is.null(input$speciesNamePicker)) {
      boxes[[length(boxes)+1]] <- column(width = 4,
                                         div(class="summary-box species-box",
